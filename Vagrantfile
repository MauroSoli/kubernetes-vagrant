...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "kubebalancer01" do |node|
    ...
    node.vm.provision "base-packages", :type => "shell", :path => "rhel/base_packages.sh"
    node.vm.provision "setup-hosts", :type => "shell", :path => "shared/setup-hosts.sh" do |s|
      s.args = [IP_NW]
    end
    node.vm.provision "setup-dns", type: "shell", :path => "shared/update-dns.sh"
    # Only Loadbalancer
  ...
  (1..NUM_ETCD_NODE).each do |i|
    config.vm.define "etcdnode0#{i}" do |node|
        ...
        # Only ETCDNode
        node.vm.provision "setup-etcd", :type => "shell", :path => "shared/setup_etcd.sh" do |s|
  ...
  (1..NUM_MASTER_NODE).each do |i|
      config.vm.define "kubemaster0#{i}" do |node|
        ...
        node.vm.provision "setup-k8s-components", :type => "shell", :path => "rhel/setup_k8s_components.sh"
        # Only MasterNode
        node.vm.provision "setup-k8s-controlplane", :type => "shell", :path => "shared/setup_k8s_controlplane.sh" do |s|
  ...
  (1..NUM_WORKER_NODE).each do |i|
    config.vm.define "kubeworker0#{i}" do |node|
        ...
        node.vm.provision "setup-k8s-components", :type => "shell", :path => "rhel/setup_k8s_components.sh"
        # Only WorkerNode
        node.vm.provision "setup-k8s-workernode", :type => "shell", :path => "shared/setup_k8s_workernode.sh" do |s|
